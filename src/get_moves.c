...
int get_w_moves (Move *moves, Pos *pos) {
	...
	if (get_short_castle_w(occupied, actual_move, pos)) { actual_move++; total_moves++; }
	if (get_long_castle_w(occupied, actual_move, pos)) { actual_move++; total_moves++; }

	piezas = pos->bitboard[2];
	while (piezas) {
		sq = __builtin_ctzll(piezas);
		piezas &= piezas - 1;
		attacks = horse_moves[sq] & ~friendly;
		while (attacks) {
			actual_move->from = sq;
			actual_move->to = __builtin_ctzll(attacks);
			attacks &= attacks - 1;
	...
	while (piezas) {
		sq = __builtin_ctzll(piezas);
		piezas &= piezas - 1;
		attacks = get_alfil_attacks(sq, friendly, enemy);
		while (attacks) {
			actual_move->from = sq;
			actual_move->to = __builtin_ctzll(attacks);
			attacks &= attacks - 1;
	...
	while (piezas) {
		sq = __builtin_ctzll(piezas);
		piezas &= piezas - 1;
		attacks = get_rook_attacks(sq, friendly, enemy);
		while (attacks) {
			actual_move->from = sq;
			actual_move->to = __builtin_ctzll(attacks);
			attacks &= attacks - 1;
	...
	while (piezas) {
		sq = __builtin_ctzll(piezas);
		piezas &= piezas - 1;
		attacks = get_queen_attacks(sq, friendly, enemy);
		while (attacks) {
			actual_move->from = sq;
			actual_move->to = __builtin_ctzll(attacks);
			attacks &= attacks - 1;
	...
	while (piezas) {
		sq = __builtin_ctzll(piezas);
		piezas &= piezas - 1;
		attacks = get_w_pawn_moves(sq, occupied);
		while (attacks) {
			actual_move->from = sq;
			actual_move->to = __builtin_ctzll(attacks);
			attacks &= attacks - 1;
...
int get_b_moves (Move *moves, Pos *pos) {
	...
	if (get_short_castle_b(occupied, actual_move, pos)) { actual_move++; total_moves++; }
	if (get_long_castle_b(occupied, actual_move, pos)) { actual_move++; total_moves++; }
	...
	while (piezas) {
		sq = __builtin_ctzll(piezas);
		piezas &= piezas - 1;
		attacks = get_b_pawn_moves(sq, occupied);
		while (attacks) {
			actual_move->from = sq;
			actual_move->to = __builtin_ctzll(attacks);
			attacks &= attacks - 1;
